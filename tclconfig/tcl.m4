...
dnl AC_CHECK_TOOL(AR, ar)
	...
	windows)
		...
		SHLIB_LD="$CCLD -shared -static-libstdc++ -static-libgcc"
		INSTALL_FLAGS="-s"
	...
	AIX-*)
		...
		AS_IF([test "$GCC" = yes], [SHLIB_LD='${CCLD} -shared'], [
	...
	Linux*)
	    ...
	    SHLIB_LD='${CCLD} -shared ${CFLAGS} ${LDFLAGS_DEFAULT}'
	    DL_OBJS="tclLoadDl.o"
	    DL_LIBS="-ldl"
	...
	OpenBSD-*)
	    SHLIB_CFLAGS="-fPIC"
	    SHLIB_LD='${CCLD} -shared ${SHLIB_CFLAGS}'
	    SHLIB_LD_LIBS='${LIBS}'
	    SHLIB_SUFFIX=".so"
	    DL_OBJS="tclLoadDl.o"
	    DL_LIBS=""
